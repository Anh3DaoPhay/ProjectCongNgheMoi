...
import 'vnpay_payment_view.dart';
...
class _CheckoutViewState extends State<CheckoutView> {
  ...
  Future<void> _submitOrder(Map<String, dynamic> cart) async {
    ...
    try {
      ...
      Globs.showHUD(status: 'Đang tạo liên kết thanh toán...');
      ...
      if (!mounted) return;
      final payUrl = payRes is Map && payRes['success'] == true ? payRes['payUrl'] as String? : null;
      final tenToaNha = buildings.firstWhere(
        (b) => b['maToaNha'] == selectedBuildingId, orElse: () => {},
      )['tenToaNha']?.toString() ?? '';

      if (payUrl != null && payUrl.isNotEmpty) {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => VNPayPaymentView(
          payUrl: payUrl, maDonHang: maDonHang, tenToaNha: tenToaNha, maToaNha: selectedBuildingId!,
        )));
      } else {
