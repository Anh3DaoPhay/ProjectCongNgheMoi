...
async function ensurePromotionsTableCompatibility() {
  await query(
    `
      ...
      ADD COLUMN IF NOT EXISTS dish_id INT NULL AFTER canteen_id,
      ADD COLUMN IF NOT EXISTS code VARCHAR(40) NULL AFTER dish_id
    `,
  );
}
...
async function getSavedVouchers(userId) {
  ...
  return query(
    `
      ...
      INNER JOIN canteens c ON c.id = p.canteen_id
...
async function saveVoucher(userId, promotionIdInput) {
  ...
  await query(
    `
      INSERT INTO user_saved_vouchers (user_id, promotion_id)
      VALUES (?, ?)
