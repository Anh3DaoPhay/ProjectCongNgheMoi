...
const OrderModel = {
    ...
    createOrder: async (orderData) => {
        const { maTaiKhoan, maToaNha, maPhong, tongTien, items } = orderData;
        ...
        return db.withTransaction(async (connection) => {
            ...
            // ── Bước 2: Tạo bản ghi donhang ──
            const [orderResult] = await connection.execute(
                `INSERT INTO donhang (maTaiKhoan, maToaNha, maPhong, tongTien, trangThaiDonHang, thoiGianDat) 
                 VALUES (?, ?, ?, ?, 'choGhepDon', NOW())`,
                [maTaiKhoan, maToaNha, maPhong, tongTien]
            );
            const newOrderId = orderResult.insertId;

            // ── Bước 3: Tạo chitietdonhang + trừ tồn kho từng món ──
            for (const item of items) {
                await connection.execute(
                    `INSERT INTO chitietdonhang (maDonHang, maMonAn, soLuong, giaTien) VALUES (?, ?, ?, ?)`,
                    [newOrderId, item.maMonAn, item.soLuong, item.giaTien]
                );

                // Trừ tồn kho (dùng điều kiện soLuongTon >= soLuong để an toàn)
    ...
    // Lấy danh sách đơn 'choXacNhan' và 'dangChuanBi' thuộc gian hàng của nhân viên
    ...
    // Lấy dữ liệu gom món cho bếp
    getKDSData: async (maGianHang) => {
        return db.query(`
            SELECT m.maMonAn, m.tenMonAn, m.hinhAnh, SUM(ct.soLuong) as tongSoLuong
            FROM donhang d
            JOIN chitietdonhang ct ON d.maDonHang = ct.maDonHang
            JOIN monan m ON ct.maMonAn = m.maMonAn
            WHERE m.maGianHang = ? 
    ...
    markKDSItemReady: async (maGianHang, maMonAn) => {
        // Cập nhật trạng thái của chi tiết đơn hàng thuộc các đơn đang chuẩn bị và của đúng gian hàng đó
        const result = await db.query(`
